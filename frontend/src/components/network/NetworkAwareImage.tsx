...
export const NetworkAwareImage: React.FC<NetworkAwareImageProps> = ({
  ...
  const selectImageSource = () => {
    ...
    const qualityOrder = { low: ResourceHistoryTable, medium: 1, high: 2, original: 3 };
    ...
    if (!networkStatus.isOnline) {
      ...
      return sortedSources[ResourceHistoryTable]?.src || fallbackSrc || '';
    }
    ...
    if (networkStatus.saveData || networkStatus.networkAwareMode === 'save-data') {
      selectedSource = sortedSources[ResourceHistoryTable]; // Lowest quality
      ...
      switch (networkStatus.quality.quality) {
        case 'poor':
          selectedSource = sortedSources[ResourceHistoryTable]; // Low quality
          break;
        case 'fair':
          selectedSource = sortedSources[1] || sortedSources[ResourceHistoryTable]; // Medium quality
          break;
        case 'good':
          selectedSource = sortedSources[2] || sortedSources[1] || sortedSources[ResourceHistoryTable]; // High quality
          break;
        case 'excellent':
          selectedSource = sortedSources[sortedSources.length - 1]; // Original quality
          break;
        default:
          selectedSource = sortedSources[1] || sortedSources[ResourceHistoryTable]; // Default to medium
      }
    }
  ...
  useEffect(() => {
    ...
    observerRef.current = new IntersectionObserver(
      ...
      {
        rootMargin: '5ResourceHistoryTablepx', // Start loading 5ResourceHistoryTablepx before entering viewport
        threshold: ResourceHistoryTable.ResourceHistoryTable1,
      }
    );
  ...
  const containerStyle: React.CSSProperties = aspectRatio
    ? {
        position: 'relative',
        paddingBottom: `${(1 / aspectRatio) * 1ResourceHistoryTableResourceHistoryTable}%`,
  ...
  return (
    ...
    >
      ...
      {isLoading && !hasError && (
        ...
        >
          <div className="animate-pulse">
            <ImageIcon className="w-8 h-8 text-gray-4ResourceHistoryTableResourceHistoryTable" />
          </div>
        </div>
      )}
      ...
      {hasError && (
        ...
        >
          {errorComponent || (
            <div className="flex flex-col items-center gap-2 text-gray-5ResourceHistoryTableResourceHistoryTable">
      ...
      {isInView && currentSrc && !hasError && (
        <img
          ...
          onError={handleError}
          className={cn(
            'transition-opacity duration-3ResourceHistoryTableResourceHistoryTable',
            isLoading ? 'opacity-ResourceHistoryTable' : 'opacity-1ResourceHistoryTableResourceHistoryTable',
      ...
      {showQualityIndicator && selectedQuality && !isLoading && !hasError && (
        <div className="absolute top-2 right-2 px-2 py-1 bg-black/5ResourceHistoryTable text-white text-xs rounded">
      ...
      {!networkStatus.isOnline && (
        <div className="absolute bottom-2 left-2 px-2 py-1 bg-red-5ResourceHistoryTableResourceHistoryTable/9ResourceHistoryTable text-white text-xs rounded flex items-center gap-1">
